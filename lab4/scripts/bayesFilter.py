...
Rosbag Py API :: http://docs.ros.org/api/rosbag/html/python/
Euler <-> Quaternion :: https://answers.ros.org/question/69754/quaternion-transformations-in-python/
Motion Model, Prob, applyGaussian :: http://ais.informatik.uni-freiburg.de/teaching/ss11/robotics/slides/06-motion-models.pdf
Github : https://github.com/muthupal007/Robotics-Algorithms/blob/master/lab4/script/test.py
'''
...
# Constants
landmarks = n.array([[125, 525], [125, 325], [125, 125],
               [425, 125], [425, 325], [425, 525]])
alpha = 15
beta = 90
threshold = 0.08 #(1.0/44100)#1/(35*35*36)

# Initial Belief is all the grids have zero probability 
# Which means robot isn't present there
# Init Grids with zeros
...
# 12,28,3 is where robot starts i.e prob is 1.
def main():
    global landmarks
    rate = rospy.Rate(10)
    rospack = rospkg.RosPack()
    lab4_path = rospack.get_path('lab4')
    bag_file_path = lab4_path + '/grid/grid.bag'
    print(bag_file_path)
    bag = rosbag.Bag(bag_file_path)
    output_file = lab4_path + 'output.txt'
    grid[12, 28, 3] = 1 # The probability of robot being here is 1
    ...
    for topic, msg, t in bag.read_messages(topics=['Movements', 'Observations']):
        if topic == 'Movements':
            ...
            string  = "Robot is at : {} with Prob value : {}".format(str(max_belief),str(n.amax(belief)))
            print(string)
            f.write(string + '\n')
...
# Getting Observations from Bag

# Observations Message : Contents
# ------------ ------- ----------
# int32 timeTag
# int32 tagNum
# float64 range
...
# Getting Movements from Bag
# Rotation 1, Translation, Rotation 2
# Calculating Yaw :: euler[2]

# Movements Message : Contents
# -------- ----------- ----------
# int32 timeTag
...
def to_position_from_belief(rot1, trans, rot2):
    global grid, new_grid, threshold
    new_grid = grid
    grid = n.copy(new_grid)
    total_probability = 0
        ...
        for b in range(0, 35):
            for c in range(0, 4):
                if new_grid[a, b, c] > threshold:
                    total_probability = prob_calc(a, b, c, rot1, trans, rot2, new_grid, grid, total_probability)
    return total_probability


def prob_calc(a, b, c, rotation1, translation, rotation2, new_grid, grid, total_probability):
    for i in range(0, 35):
                for j in range(0, 35):
                        for k in range(0, 4):
